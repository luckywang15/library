   ...
   <td height="22">
    <?php
     ...
     //urldecode()函数将 URL 编码后字符串还原成未编码的样子。编码使用 %## 的格式
      $sql="select * from books order by id desc";  //倒序排列
     }else{
      $sql="select * from books where type='".urldecode($_GET['proid'])."'";
     ...
     if(!urldecode(@$_GET["proid"])){
       ...
       $sql="select * from books where type='".urldecode($_GET['proid'])."' order by id desc limit $startno,$pagesize";
     }
     $rs=mysqli_query($link,$sql);
     ?>
     <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
      ...
      <form action="batchborrow.php" method="post">
       <?php
         if(!empty($rs)){
        ...
        <tr>
         ...
         <td height="30" align="center" bgcolor="#FFFFFF"><input id="choose_id[]" name="choose_id[]" type="checkbox" value =<?php echo $rows["id"];?>><?php echo $rows["id"];?></td>
